 ...
 * */
pqs.directive("antiLoader", function ($rootScope) {
        return function ($scope, element, attrs) {
            $scope.$on("loader_show", function (event, args) {
                if(!angular.isUndefined(args) && !angular.isUndefined(args.loaderFor) && !angular.isUndefined(attrs.antiLoader)){
                    if(args.loaderFor == attrs.antiLoader){
                        element.hide();
                    }
                }
            });
