...
angular.module('mgo-angular-wizard').directive('wzStep', function() {
    return {
        ...
        scope: {
            wzTitle: '@',
            title: '@',
            questionTitle: '@',//handysolver
            questionDescription:'@', //handysolver
...
angular.module('mgo-angular-wizard').directive('wizard', function() {
    return {
        ...
        controller: ['$scope', '$element', '$log', 'WizardHandler', function($scope, $element, $log, WizardHandler) {
            ...
            $scope.nextVisible = function(step){
                if(step.isVisible == "false"){
                    console.log('skip');
                    var index = _.indexOf($scope.steps , step);

                    if(($scope.currentStepNumber()-1) < index){//going forward
                        index = index + 1;
                    }
            ...
            this.currentStepPosition = function() {
                var count = 0;
                var currentStepIndex = _.indexOf($scope.steps , $scope.selectedStep);
                angular.forEach($scope.steps, function(step, key) {
                    if(step.isVisible == "true" && key <= currentStepIndex)
                        count++;
                });
                return count;
            };
