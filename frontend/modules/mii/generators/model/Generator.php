...
namespace frontend\modules\mii\generators\model;

use frontend\modules\mii\generators\CommonGenerator;
...
class Generator extends CommonGenerator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $params = [
                ...
                'selectValueArray'=>$this->generateSelectValueArray($tableSchema),
            ];
            $files[] = new CodeFile(
    ...
    public function generateSelectValueArray($table){
        $finalData=[];
        $container=['fieldName'=>null,'fieldSelectValues'=>[]];
        $data=$this->newFormData;
        foreach ($data as $selectVal){
            if($selectVal['type']=='select'){
                $selectArray=$selectVal['values'];
                $newSelectArray=[];
                foreach ($selectArray as $key=>$selectData){
                    $value=(string)$selectData['label'];
//                    $newSelectArray[$key]=$selectData['label'];
                    $newSelectArray[$value]=$value;
                }
                array_push($finalData,['fieldName'=>$selectVal['name'],'fieldSelectValues'=>$newSelectArray]);
            }
        }
    ...
    public function generateRules($table)
    {
        ...
        if($this->modify){
            foreach ($this->extendedRulesArray as $extendedRule){
                $rules[]="['".$extendedRule['name']."','".$extendedRule['value']."']";
            }
